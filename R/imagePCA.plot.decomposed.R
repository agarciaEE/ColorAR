...
#' groups <- sample(letters[1:3], length(imgPCA12$images), replace = T)
...
imagePCA.plot.decomposed <- function(imagePCA, tree, groups = NULL, scale = F, PCA.cols = NULL, tree.cols = NULL,
  ...
  if (is.null(tree.cols)) { tree.cols <- setNames(rep("transparent", ntips), tree$tip.label) }
  if (is.null(PCA.cols)) { PCA.cols <- setNames(rep("black", nrow(df)), rownames(df)) }
  ...
  points(df[,PCx], df$vpos, col = PCA.cols[rownames(df)], ...)
  ...
  rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "grey90")
  abline(v = 0, lty = 2, lwd = 2, col = "white")
  points(df[,PCy], df$vpos, col = PCA.cols[rownames(df)], ...)
  ...
  if(!is.null(PCA.legend)) legend("left", legend = names(PCA.legend), fill= PCA.legend, bg = "transparent",  cex=3, box.lty = 0, bty = "n", horiz = T)
  if(!is.null(tree.legend)) legend("right", legend = names(tree.legend), fill= tree.legend, bg = "transparent",
